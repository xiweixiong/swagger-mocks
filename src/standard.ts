...
// 兼容性代码，将老的 datatype 转换为新的。
function dataType2StandardDataType(dataType: DataType, originName: string, defNames: string[], compileTemplateKeyword?: string) {
...
export class Property extends Contextable {
  ...
  toPropertyCode(surrounding = Surrounding.typeScript, hasRequired = false, optional = false) {
    ...
    const fieldTypeDeclaration = surrounding === Surrounding.javaScript ? '' : `${optionalSignal}: ${this.dataType.generateCode(this.getDsName())}`
