...
import { BaseClass, Interface, Mod, Property, StandardDataSource, StandardDataType } from '../standard'
import { getIdentifierFromOperatorId, getIdentifierFromUrl, getMaxSamePath, hasChinese, toDashCase, transformCamelCase, transformModsName } from '../utils'
...
class Schema {
  ...
  static parseSwaggerSchema2StandardDataType(schema: Schema, defNames: string[], classTemplateArgs = [] as StandardDataType[], compileTemplateKeyword?: string) {
    ...
    if (type === 'object') {
      if (additionalProperties) {
        const typeArgs = [new StandardDataType(), Schema.parseSwaggerSchema2StandardDataType(additionalProperties, defNames, classTemplateArgs, compileTemplateKeyword)]
...
class SwaggerInterface {
  ...
  static transformSwaggerInterface2Standard(inter: SwaggerInterface, usingOperationId: boolean, samePath: string, defNames: string[] = [], compileTempateKeyword?: string) {
    let name = ''
...
export function parseSwaggerMods(swagger: SwaggerDataSource, defNames: string[], usingOperationId: boolean, compileTempateKeyword?: string) {
...
export function transformSwaggerData2Standard(swagger: SwaggerDataSource, usingOperationId = true, originName = '') {
  const draftClasses = _.map(swagger.definitions, (def, defName) => {
    ...
    return { name: defNameAst.name, defNameAst, def }
  })
  ...
  const baseClasses = draftClasses.map((clazz) => {
    ...
    const props = _.map(properties, (prop, propName) => {
      const { $ref, description, type, items, additionalProperties, format } = prop
      ...
      return new Property({ dataType, name: propName, description, required, format })
    })

    return new BaseClass({ description, name: clazz.name, properties: props, templateArgs })
  })
