...
exports.SwaggerV2Reader = exports.transformSwaggerData2Standard = exports.parseSwaggerMods = exports.SwaggerDataSource = exports.parseSwaggerEnumType = void 0;
    ...
    static parseSwaggerSchema2StandardDataType(schema, defNames, classTemplateArgs = [], compileTemplateKeyword) {
        ...
        if (type === 'object') {
            if (additionalProperties) {
                const typeArgs = [new standard_1.StandardDataType(), Schema.parseSwaggerSchema2StandardDataType(additionalProperties, defNames, classTemplateArgs, compileTemplateKeyword)];
...
function parseSwaggerMods(swagger, defNames, usingOperationId, compileTempateKeyword) {
    ...
    const mods = swagger.tags
        ...
        const modInterfaces = allSwaggerInterfaces.filter((inter) => {
            ...
            return inter.tags.includes(tag.name) || inter.tags.includes(tag.name.toLowerCase()) || inter.tags.includes(tag.description.toLowerCase()) || inter.tags.includes(utils_1.toDashCase(tag.description));
        });
...
function transformSwaggerData2Standard(swagger, usingOperationId = true, originName = '') {
    ...
    const baseClasses = draftClasses.map((clazz) => {
        ...
        const props = _.map(properties, (prop, propName) => {
            ...
            return new standard_1.Property({ dataType, name: propName, description, required, format });
        });
        return new standard_1.BaseClass({ description, name: clazz.name, properties: props, templateArgs });
    });
